...
}//La función "selectPackage" se utiliza para seleccionar un paquete específico dentro de un sistema. Toma como argumento el índice del paquete que se desea seleccionar y lo guarda en un arreglo llamado "selectedPackages". Además, establece un valor inicial de $10 para el total del paquete seleccionado y luego llama a la función "updateSummary" para actualizar el resumen.
...
function selectDestination(destinationIndex) {
    ...
    updateSummary(); // Actualizar el resumen
}//Esta función se encarga de seleccionar un destino a partir de un índice dado. El índice del destino seleccionado se guarda en un arreglo llamado "selectedDestinations". Además, se establece un total inicial de $10 para ese destino. Luego, se llama a la función "updateSummary()" para actualizar el resumen.
...
function selectTransportation(transportationIndex) {
    ...
    updateSummary(); // Actualizar el resumen
}//La función "selectTransportation" se utiliza para seleccionar una opción de transporte. Toma como argumento el índice de la opción de transporte seleccionada y guarda este índice en una variable llamada "selectedTransportation". Además, establece el total inicial en $10 para la opción de transporte seleccionada y luego llama a la función "updateSummary" para actualizar el resumen.
...
function addToSummary(item, category) {
    ...
    summaryItems.appendChild(listItem); // Agrega el elemento de lista al resumen appendChild es un método en JavaScript que se utiliza para agregar un nodo como hijo de otro nodo existente en el árbol DOM (Document Object Model). Se puede utilizar para agregar elementos HTML, como elementos de lista, párrafos, imágenes, etc., como hijos de otros elementos existentes en el documento HTML.
}
